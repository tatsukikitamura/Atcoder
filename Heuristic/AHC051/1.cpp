    ...
    while (!q.empty()) {
        ...
        if (best_dest_node_id == -1) {
            ...
            for (int i = 0; i < N; ++i) {
                ...
                if (fallback_dest_id == -1 || d_sq < min_dist_sq) {
                    min_dist_sq = d_sq;
                    fallback_dest_id = i;
                }
            }

            if (fallback_dest_id != -1) {
                best_dest_node_id = fallback_dest_id;
            } else {
                // フォールバック先が見つからない場合、最も確率の高いゴミに対応する処理装置に設定
                int target_disposal_loc = -1;
                for (int i = 0; i < N; ++i) {
                    if (d[i] == max_p_idx) {
                        target_disposal_loc = i;
                        break;
                    }
                }
                // 原則見つかるはずだが、念のためデフォルト値として0を設定
                best_dest_node_id = (target_disposal_loc != -1) ? target_disposal_loc : 0;
            }
        }
        // ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
