...
// 点qが線分pr上にあるか判定（3点が同一直線上にある場合用）
bool on_segment(Point p, Point q, Point r) {
    return (q.x <= std::max(p.x, r.x) && q.x >= std::min(p.x, r.x) &&
            q.y <= std::max(p.y, r.y) && q.y >= std::min(p.y, r.y));
}

// 2つの線分 p1p2 と q1q2 が（端点を含め）交差するかを判定する
...
double calculate_info_gain(const std::vector<double>& current_probs, int sorter_k, int N, const std::vector<std::vector<double>>& p_mat) {
    ...
    double h1 = entropy(probs1);
    double h2 = entropy(probs2);
...
int main() {
    ...
    std::sort(disposal_indices.begin(), disposal_indices.end(), [&](int a, int b) {
        if (disposal_coords[a].y != disposal_coords[b].y) {
            return disposal_coords[a].y < disposal_coords[b].y;
        }
        return disposal_coords[a].x < disposal_coords[b].x;
    });

    // ソート後のインデックスに対して、ゴミの種類を0から割り当てる
    for (int i = 0; i < N; ++i) {
        // disposal_indices[i] がソート後のi番目の「場所のインデックス」
        // これにゴミの種類 i を割り当てる
        d[disposal_indices[i]] = i;
    }

    // 出力変数
    int s = -1; // 搬入口の接続先
    std::vector<int> c_k(M, -1);
    std::vector<int> c_v1(M, -1);
    std::vector<int> c_v2(M, -1);

    // 構築のための補助変数
    std::vector<bool> sorter_loc_used(M, false);
    std::vector<std::pair<Point, Point>> belts;
    std::queue<std::tuple<int, int, std::vector<double>>> q;

    std::vector<double> initial_probs(N, 1.0);
    q.emplace(-1, 1, initial_probs);

    const double PURITY_THRESHOLD = 0.90;

    // ---- 3. ネットワーク構築ループ ----
    while (!q.empty()) {
        auto [parent_loc_idx, exit_num, current_probs] = q.front();
        q.pop();

        Point source_p = (parent_loc_idx == -1) ? Point{0, 5000} : sorter_coords[parent_loc_idx];

        double sum_p = 0.0, max_p = 0.0;
        int max_p_idx = -1;
        for (int i = 0; i < N; ++i) {
        ...
        if (purity > PURITY_THRESHOLD) {
            ...
            if (target_disposal_loc != -1) {
                ...
                for (const auto& belt : belts) {
                    if (segments_intersect(source_p, dest_p, belt.first, belt.second)) {
                        intersects = true;
                        break;
                    }
                }
                if (!intersects) {
                    best_dest_node_id = target_disposal_loc;
                }
            }
        }
        ...
        if (best_dest_node_id == -1) {
            ...
            int best_sorter_loc = -1;
            int best_sorter_type = -1;
            ...
            if (best_sorter_loc != -1) {
                ...
                q.emplace(best_sorter_loc, 1, probs1);
                q.emplace(best_sorter_loc, 2, probs2);
            }
        }
        
        if (best_dest_node_id == -1) {
            long long min_dist_sq = -1;
            int fallback_dest_id = -1;
            ...
            for (int i = 0; i < N; ++i) {
                ...
                long long dx = source_p.x - dest_p.x;
                long long dy = source_p.y - dest_p.y;
                long long dist_sq = dx*dx + dy*dy;
                
                if (fallback_dest_id == -1 || dist_sq < min_dist_sq) {
                    min_dist_sq = dist_sq;
                    fallback_dest_id = i;
                }
            }
        ...
        if (parent_loc_idx == -1) {
            ...
            if (exit_num == 1) c_v1[parent_loc_idx] = best_dest_node_id;
            else c_v2[parent_loc_idx] = best_dest_node_id;
        }
    }
