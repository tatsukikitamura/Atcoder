...
# --- 4. メインロジック (経路決定) ---
# ### CHANGED ###: 戦略2 (交差ヒューリスティック) に基づいて書き換え

# 4.1. 全区間の最短経路と「交差カウント」を計算
segments_info = []
all_shortest_paths = [] # 全 k についての最短経路(パスリスト)
all_shortest_path_sets = [] # ↑を set にしたもの
shortest_lengths = []
...
for k in range(K - 1):
    ...
    all_shortest_paths.append(path_k)
    all_shortest_path_sets.append(set(path_k))
    shortest_lengths.append(X_k)
    if X_k != float('inf'):
        total_shortest_steps_X += X_k

# 4.1.2. 交差カウントを計算
for k in range(K - 1):
    path_k_set = all_shortest_path_sets[k]
    ...
    for i in range(K - 1):
        ...
        path_i_set = all_shortest_path_sets[i]
        
        # 集合の積 (intersection) が空 (isdisjoint) でないかチェック
        if not path_k_set.isdisjoint(path_i_set):
            intersection_count += 1
            
    segments_info.append({
        'k': k,                 # 区間番号
        'X_k': shortest_lengths[k], # 最短経路長
        'path_k': all_shortest_paths[k], # 最短経路リスト (フォールバック用)
        'intersection': intersection_count # 交差カウント
    })

# 4.2. 交差カウントが「多い」順 (降順) にソート
sorted_segments = sorted(segments_info, key=lambda x: x['intersection'], reverse=True)

# 4.3. ソートされた順に経路を決定 (このブロックは戦略1と変更なし)
