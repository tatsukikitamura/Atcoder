...
inline double quick_eval(const BState& bs, int tx, int ty, int turn) {
    ...
    if (lev == 1) {
        ...
        if (is_top && phase > 0.4)
            base += v * 0.8 * min(1.0, (phase - 0.4) * 2.0);
        return base;
    }
...
double evaluate(const BState& bs, int turn, int depth) {
    ...
    // [ATTACK] Small extra penalty for top scorer's V*L
    // Nudges search to prefer states where top scorer is weaker
    double top_vl = bs.cached_top_vl;
    double w7 = (phase > 0.6) ? 0.1 * min(1.0, (phase - 0.6) / 0.3) : 0.0;

    double result = w1 * vl + w2 * fut + w3 * saf - w6 * ovl - w7 * top_vl;
